...
# Inject JavaScript file into Streamlit and add CSS for tooltips
components.html("""
    <script src="app.js"></script>
    <style>
        .section-header { font-weight: bold; margin-top: 20px; cursor: pointer; }
    </style>
""", height=0)
...
if user_menu == 'Overall Analysis':
    ...
    st.write('<div class="section-header">Participating Nations over the years</div>', unsafe_allow_html=True)
    st.plotly_chart(fig)

    events_over_time = helper.data_over_time(df, 'Event')
    fig = px.line(events_over_time, x="Year", y="count")
    st.write('<div class="section-header">Events over the years</div>', unsafe_allow_html=True)
    st.plotly_chart(fig)

    athlete_over_time = helper.data_over_time(df, 'Name')
    fig = px.line(athlete_over_time, x="Year", y="count")
    st.write('<div class="section-header">Athletes over the years</div>', unsafe_allow_html=True)
    st.plotly_chart(fig)

    st.write('<div class="section-header">No. of Events over time (Every Sport)</div>', unsafe_allow_html=True)
    fig, ax = plt.subplots(figsize=(20, 20))
    x = df.drop_duplicates(['Year', 'Sport', 'Event'])
    sns.heatmap(x.pivot_table(index='Sport', columns='Year', values='Event', aggfunc='count').fillna(0).astype(int), annot=True)
    st.pyplot(fig)

    st.write('<div class="section-header">Most Successful Athletes</div>', unsafe_allow_html=True)
...
if user_menu == 'Country-wise Analysis':
    ...
    if pt.empty:
        st.write("No data to display in the heatmap.")
    else:
        fig, ax = plt.subplots(figsize=(20, 20))
...
if user_menu == 'Athlete wise Analysis':
    ...
    famous_sports = ['Basketball', 'Judo', 'Football', 'Tug-Of-War', 'Athletics',
                     ...
                     'Table Tennis', 'Baseball', 'Rugby Sevens', 'Triathlon', 'Polo', 'Ice Hockey']
